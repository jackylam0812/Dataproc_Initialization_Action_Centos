...
# Database admin user password used to create the database and user.
...
EOF
  ...
  # # Check if hue database is initialized.
  # if ! mysql -u hue -p${hue_password} -e ''; then
  #   mysql -u root -pjacky1983 -e "
  #     CREATE USER IF NOT EXISTS 'hue' IDENTIFIED BY '${hue_password}';"
  # fi
  # if ! mysql -u hue -p${hue_password} -e "use hue"; then
  #   # Initialize a hue DB
  #   mysql -u root -pjacky1983 -e \
  #     "CREATE DATABASE if not exists hue;
  #      GRANT ALL PRIVILEGES ON hue.* TO 'hue';"
